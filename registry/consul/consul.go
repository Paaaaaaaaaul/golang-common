...
func configure(c *consulRegistry, opts ...registry.Option) {
	...
	if c.options.Context != nil {
		...
		if co, ok := c.options.Context.Value("consul_config").(*consul.Config); ok {
			config = co
		}
		if cn, ok := c.options.Context.Value("consul_connect").(bool); ok {
			c.connect = cn
		}

		// Use the consul query options passed in the options, if available
		if qo, ok := c.options.Context.Value("consul_query_options").(*consul.QueryOptions); ok && qo != nil {
			c.queryOptions = qo
		}
		if as, ok := c.options.Context.Value("consul_allow_stale").(bool); ok {
	...
	if c.options.Secure || c.options.TLSConfig != nil {
		config.Scheme = "https"
		// We're going to support InsecureSkipVerify
		config.HttpClient.Transport = newTransport(c.options.TLSConfig)
	}
...
func (c *consulRegistry) Register(s *registry.Service, opts ...registry.RegisterOption) error {
	...
	if c.options.Context != nil {
		if tcpCheckInterval, ok := c.options.Context.Value("consul_tcp_check").(time.Duration); ok {
			regTCPCheck = true
...
func (c *consulRegistry) KeepAliveOnce() error {

	return nil
}

func (c *consulRegistry) KeepAlive(ttl time.Duration) {
