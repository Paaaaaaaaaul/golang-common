...
func main() {
	...
	response, err := base_server_pay.SelectPayMethods(mchId, base_server_pay.CURRENCY_CC, "", signKey, base_server_pay.VERSION)
	if err != nil {
		println(err.String())
	} else {
		fmt.Printf("查询支付方式：[%v]\n", response)
	...
	response1, err1 := base_server_pay.SelectPayChannels(mchId, "wechat_qr_code_cny",
		"100", base_server_pay.CURRENCY_CC, base_server_pay.CURRENCY_CC, signKey, base_server_pay.VERSION)
	if err1 != nil {
		println(err1.String())
	} else {
		fmt.Printf("查询支付通道：[%v]\n", response1)
	...
	response2, err2 := base_server_pay.SubmitPayOrder(mchId, "wechat_qr_code_cny", "4", base_server_pay.RandString(10),
		"100", base_server_pay.CURRENCY_CC, base_server_pay.CURRENCY_CC, "rrr", "ee", "http://www.baidu.com", signKey, base_server_pay.VERSION)
	if err2 != nil {
		println(err2.String())
	} else {
		fmt.Printf("支付下单：[%v]\n", response2)
	...
	response3, err3 := base_server_pay.GenerateUnionPayUrl(mchId, base_server_pay.CURRENCY_CC, "222", strconv.FormatInt(time.Now().Unix(), 10),
		"100", "http://www.baidu.com", "http://www.baidu.com", "", signKey, base_server_pay.VERSION)
	if err3 != nil {
		println(err3.String())
	} else {
		fmt.Printf("聚合支付链接：[%v]\n", response3)
	...
	response4, err4 := base_server_pay.SelectWithdrawMethods(mchId, base_server_pay.CURRENCY_CC, signKey, base_server_pay.VERSION)
	if err4 != nil {
		println(err4.String())
	} else {
		fmt.Printf("提现方式：[%v]\n", response4)
	...
	response5, err5 := base_server_pay.SelectWithdrawChannels(mchId, "wdw_bank_card_pay_cny", "1", base_server_pay.CURRENCY_CC,
		base_server_pay.CURRENCY_CC, "", signKey, base_server_pay.WALLET_TYPE, base_server_pay.VERSION)
	if err5 != nil {
		println(err5.String())
	} else {
		fmt.Printf("提现通道：[%v]\n", response5)
	...
	response7, err7 := base_server_pay.QueryWithdrawOrder(mchId, "TYCGWSBNKY", "210011571217172904", signKey, base_server_pay.VERSION)
	if err7 != nil {
		println(err7.String())
	} else {
		fmt.Printf("代付查询：[%v]\n", response7)
