...
// 账户日志列表
// post account/accountLogListAll
//
//	类型枚举:
//	1	可用-加
//	2	可用-减
...
//	1001 参数错误
//	2003 账户不存在
func AccountLogListAll(orgId int, userId int64, userIds string, opType OpType, bsType int, currency string, beginTime, endTime int, page, limit int, accountType ...int) ([]*LogList, *base_server_sdk.Error) {
	if orgId <= 0 || opType < 0 || page <= 0 || limit > 1000 {
		return nil, base_server_sdk.ErrInvalidParams
	}
